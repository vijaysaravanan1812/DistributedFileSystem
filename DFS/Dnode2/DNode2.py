...
IP,PORT,PATH = "127.0.0.1",8000,r'C:\Temp\TMP\DFS\Dnode2'


FTP: file -> ftpserver2.py || port: 8001  (Dnode port + 1)
...
global IP,PORT,PATH
IP,PORT,PATH = "127.0.0.1",8000,r'/tmp/DFS/Dnode2'
...
if __name__ == "__main__":
    t2 = ThreadedServer(DNServer, hostname=IP, port=PORT, protocol_config={'allow_public_attrs': True})
    ...
    subprocess.call("start cmd /K python ftpserver2.py", shell=True) # this opens the ftpserver2.py file in a new console window (shell=True) so that we can view the FTP logs
